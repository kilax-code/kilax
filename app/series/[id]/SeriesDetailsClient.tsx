...
import { getSeriesByIdClient } from "@/lib/api-client";
...
export default function SeriesDetailsClient({ id, initialSeries }: Props) {
  ...
  const [selectedDownloadEpisode, setSelectedDownloadEpisode] =
    useState<any>(null);
  ...
  useEffect(() => {
    async function fetchSeriesData() {
      ...
      const seriesData = initialSeries || (await getSeriesByIdClient(id));
      ...
      try {
        const { getEpisodes, getRelatedSeriesByGenre } = await import(
          "@/lib/api"
        );
        const [episodesData, relatedData] = await Promise.all([
          getEpisodes(id, selectedSeason),
          getRelatedSeriesByGenre(id, seriesData.genre_ids || []),
        ]);
    ...
    fetchSeriesData();
  }, [id, selectedSeason, initialSeries]);
  ...
  const handleWatch = async (episodeNumber?: number) => {
    ...
    if (episodeNumber) {
      router.push(
        `/player?id=${series.id}&type=series&season=${selectedSeason}&episode=${episodeNumber}`
      );
    } else {
      const episode =
        episodes.find((e) => e.episode_number === 1) || episodes[0];
      if (episode) {
        router.push(
          `/player?id=${series.id}&type=series&season=${selectedSeason}&episode=${episode.episode_number}`
        );
      } else {
        router.push(
          `/player?id=${series.id}&type=series&season=${selectedSeason}&episode=1`
        );
      }
    }
  };
  ...
  const handleDownload = async (episode?: any) => {
    ...
    if (episode) {
      ...
      setSelectedDownloadEpisode(episodes[0]);
      setShowDownloadModal(true);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white">
      <HeroDetail
        ...
        coverImage={
          ...
          `https://via.placeholder.com/1920x1080/1f2937/f97316?text=${encodeURIComponent(series.title)}`
        }
        onWatch={() => handleWatch()}
        ...
        {related.length > 0 ? (
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[2/3] rounded-lg bg-gray-800/30 animate-pulse"
              ></div>
            ))}
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
          <div className="bg-gray-900 p-8 rounded-xl border border-orange-400 shadow-xl max-w-sm w-full text-center">
            ...
            <p className="text-gray-300 mb-4 line-clamp-2">
              {series.title} -{" "}
              {selectedDownloadEpisode.title ||
                `Episode ${selectedDownloadEpisode.episode_number}`}
            </p>
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const cleanTitle = series.title
                    ? series.title.replace(/[^a-zA-Z0-9\s\-_.]/g, "").trim()
                    : "series";
                  ...
                  alert(
                    "Download is not available for this episode right now. Please try again."
                  );
                }
              }}
            >
