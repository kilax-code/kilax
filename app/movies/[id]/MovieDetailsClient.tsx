...
import { getMovieByIdClient } from "@/lib/api-client";
...
export default function MovieDetailsClient({ id, initialMovie }: Props) {
  const router = useRouter();
  const { user, loading: authLoading, isPremium } = useAuth();

  const [movie, setMovie] = useState<any | null>(initialMovie || null);
  ...
  useEffect(() => {
    (async () => {
      if (!user) {
        setIsStandardPremiumUser(false);
        return;
      }
      const subscription = await (
        await import("@/lib/subscriptions")
      ).getUserSubscription(user.id);
      const { isStandardPremium: checkStandard } = await import(
        "@/lib/isStandardPremium"
      );
  ...
  useEffect(() => {
    async function fetchMovieData() {
      if (initialMovie) {
        // Already have data from server, just load related
        try {
          const { getRelatedMoviesByGenre } = await import("@/lib/api");
          const relatedData = await getRelatedMoviesByGenre(
            id,
            initialMovie.genre_ids || []
          );
      ...
      const movieData = await getMovieByIdClient(id);
      if (!movieData) {
  ...
  const handleWatch = async () => {
    ...
    if (playUrl) {
      router.push(
        `/player?id=${movie.id}&type=movie&url=${encodeURIComponent(playUrl)}`
      );
    } else {
    ...
    <div className="min-h-screen bg-black text-white">
      <HeroDetail
        ...
        coverImage={
          ...
          movie.backdrop_path ||
          movie.thumbnail_url ||
          `https://via.placeholder.com/1920x1080/1f2937/f97316?text=${encodeURIComponent(movie.title)}`
        }
        onWatch={handleWatch}
              ...
              onClick={async () => {
                ...
                try {
                  const cleanTitle = movie.title
                    ? movie.title.replace(/[^a-zA-Z0-9\s\-_.]/g, "").trim()
                    : "movie";
                  ...
                  alert(
                    "Download is not available for this movie right now. Please try again."
                  );
                }
              }}
            >
